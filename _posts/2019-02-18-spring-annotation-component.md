...
&emsp;a、@Import(要导入到容器中的组件)；容器中就会自动注册这个组件，id默认是全类名  
&emsp;b、ImportSelector:返回需要导入的组件的全类名数组；  
&emsp;c、ImportBeanDefinitionRegistrar:手动注册bean到容器中  
4、使用Spring提供的 FactoryBean（工厂Bean）;  
&emsp;a、默认获取到的是工厂bean调用getObject创建的对象  
&emsp;b、要获取工厂Bean本身，我们需要给id前面加一个&  